...
def agrandar_cuadrado(pentagrama:list[list[int]], posiciones_cuadrado:list[int], UMBRAL_NEGRO:int, GROSOR:int, AUMENTO_MINIMO_LATERALES:int, AUMENTO_MINIMO_ARRIBA_ABAJO:int, posiciones_pentagrama:list, DETECTAR_CORCHEAS:bool) -> tuple[list[int], list[int]]:
    ...
    if DETECTAR_CORCHEAS:#and not posiciones_cuadrado[-1] - posiciones_cuadrado[0] > 0.8*(posiciones_pentagrama[-1] - posiciones_pentagrama[0]):
...
def recorrer_pentagrama(pentagrama:list, distancia:int, UMBRAL_NEGRO:int, GROSOR:int, posiciones_pentagrama:list[int,int], DETECTAR_CORCHEAS: bool, PORCENTAJE_DETECTAR_NOTA:float) -> list[tuple]:
    '''
    ...
    while posicion_horizontal < len(pentagrama[0]):  # Recorre el pentagrama horizontalmente
        ...
        while posicion_vertical < len(pentagrama):  # Recorre el pentagrama verticalmente
            ...
            if len(cuadrado) == distancia and (cuadrado < UMBRAL_NEGRO).sum() > PORCENTAJE_DETECTAR_NOTA*(cuadrado <= 255).sum(): 
                ...
                posiciones_nuevas, posciiones_rectangulo = agrandar_cuadrado(pentagrama, posiciones, UMBRAL_NEGRO, GROSOR, AUMENTO_MINIMO_LATERALES, AUMENTO_MINIMO_ARRIBA_ABAJO, posiciones_pentagrama, DETECTAR_CORCHEAS)  
