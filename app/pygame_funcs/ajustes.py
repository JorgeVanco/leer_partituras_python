...
def set_porcentaje_diferenciar_negra_blanca(value, AJUSTES):
    AJUSTES.PORCENTAJE_DETECTAR_NOTA = np.round(value, 3)
...
def draw_menu(menu, screen, AJUSTES, AJUSTES_DEFAULT, complete_path):
    menu.clear()
    menu.add.range_slider("Porcentaje para detectar una nota: "+str(AJUSTES.PORCENTAJE_DETECTAR_NOTA), default=AJUSTES.PORCENTAJE_DETECTAR_NOTA, range_values=(0, 1), increment=0.1, onchange = lambda value:set_porcentaje_detectar_nota(value, AJUSTES))
    menu.add.range_slider("Porcentaje para diferenciar entre negras y blancas: "+str(AJUSTES.PORCENTAJE_DIFERENCIAR_NEGRA_BLANCA), default=AJUSTES.PORCENTAJE_DIFERENCIAR_NEGRA_BLANCA, range_values=(0, 1), increment=0.1, onchange = lambda value:set_porcentaje_diferenciar_negra_blanca(value, AJUSTES))
